...
- It stores full-size momentum and second-moment buffers in the sketch, which conflicts with UsuiTrack's active matrix-state invariant: no full-size first or second moments on the main matrix path.
- Its memory claims were prospective design estimates, not validated local benchmark results.
- Its target hardware/model framing predates the current UsuiTrack product target and harness defaults.
...
Only reopen this line if there is a concrete question that UsuiTrack does not answer, such as a validated need for adaptive channel scaling in the projected path. Any revival should start as a new design note that compares directly against the current UsuiTrack invariants rather than mutating this historical sketch into a second competing truth.
