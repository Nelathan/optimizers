...
- **Projected-activation MLP backward is the first real peak-VRAM win.** Parameter hooks were too late, but custom LFM MLP backward with activation-facing/storage-right bases lowered steady LFM-350M `bs4 × seq1024` peak from the old full-gradient neighborhood to `~3.99 GB` for 10 measured projected steps. The first 200-step survival run completed at `~21k tokens/s` but peaked at `~5.26 GB` because burst refresh fallback materialized full gradients; opt-in layer-staggered refresh brought the 200-step peak back near steady state at `~4.09 GB` with essentially unchanged throughput.
...
- **Extend refresh smoothing beyond the first layer-staggered cut.** Opt-in layer-staggered refresh proved that simple phase offsets can preserve most of the projected-MLP memory win across a refresh boundary. Next check whether this still holds with attention projected backward, activation checkpointing, and longer quality runs before making it a default.
