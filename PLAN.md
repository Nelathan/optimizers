...
- **Build on projected activation by using checkpointing as a measured substrate, not a slogan.** At `2e-4`, corrected full `lfm` activation projection and a no-activation all-right control matched closely at 100 steps, separating custom backward math from side-policy geometry. LFM2 decoder-layer checkpointing now behaves as intended after the harness repair, so the practical next question is how much projected activation still buys on top of real layer checkpointing and whether any remaining full-activation saves are worth cutting without dropping into kernel work.
...
- **Compile projected backend after the checkpoint substrate is stable.** The current custom autograd/wrapper path is eager and uncompiled. Measure compile only after the repaired checkpoint path, projected wrappers, and refresh behavior are coherent; do not let performance polish outrun correctness.
