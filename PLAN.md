...
The small-model faithful SYNTH lane has mostly done its job, but repaired LFM2 checkpointing reopened the useful token/rank regime. The current 350M mainline is residual-facing one-sided projection, stable side-Gram `eigh`, burst refresh `100`, Aurora `pp=2/ns=5`, CCE, repaired activation checkpointing, and larger rank/token settings than the old `bs8/rank64` continuity lane.
...
- **Test fp32 projected moments before high-beta long runs.** Rank256 and larger token batches make higher moment beta (`0.96–0.99`) plausible, but bf16 projected moments may quietly lose useful accumulator precision over long runs. First priority is fp32 `projected_exp_avg`; fp32 basis/projection math is lower priority because init/refresh already use fp32 work internally and the memory cost is larger. This likely will not show up in short smokes, but it matters for 24h stability.
