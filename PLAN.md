...
- **Clean the Grassmann refresh signal before more tuning.** Basis init normalizes the full gradient before side-Gram `eigh`; Grassmann refresh currently forms its tangent from raw full-gradient scale. Larger rank/batches exposed smooth but large chordal motion and occasional rank64 blips. The next algorithmic code experiment should normalize or otherwise scale-control the Grassmann refresh input like init before tangent formation. Do not conflate that with more LR, beta, or rank sweeps.
