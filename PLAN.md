...
- **Build on projected activation by removing remaining full-activation costs.** At `2e-4`, checkpointed full `lfm` activation projection and a no-activation all-right control matched closely at 100 steps, separating custom backward math from side-policy geometry. Treat that as a passed milestone, not a lane to maintain. The fused MLP no longer saves full `gate_pre`/`up`; next find the remaining no-checkpoint peak drivers and cut them without reintroducing full-gradient materialization.
