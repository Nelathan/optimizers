...
- **Run corrected-LR quality curves for activation-facing geometry.** At `2e-4`, full `lfm` activation projection and a no-activation all-right control matched closely at 100 steps, separating custom backward math from side-policy geometry. Next compare residual-facing, all-right, MLP-only activation projection, and full `lfm` activation projection under the corrected LR before promoting any activation-projection default.
