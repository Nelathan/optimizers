...
The small-model faithful SYNTH lane has mostly done its job, but repaired LFM2 checkpointing reopened the useful token/rank regime. The current 350M mainline is residual-facing one-sided projection, stable side-Gram `eigh`, burst refresh `100`, Aurora `pp=2/ns=5`, CCE, repaired activation checkpointing, per-matrix projected-gradient clipping at `2.0`, and larger rank/token settings than the old `bs8/rank64` continuity lane.

Treat the 350M faithful SYNTH setup as both a regression harness and the current optimizer-shape tuner. Use it when a new code path, scale step, rank/token setting, or comparator needs a known-good control.
...
- **Rank256 plus smaller batches is the current best 350M default candidate.** A rank256 `bs32`, LR `6e-4`, 500-step interrupted/partial run produced strong smooth target movement (`1.749001 → 1.699121` from step 100 to 500) but high source (`3.102894` at step 500), consistent with “more signal and too much update scale.” A completed rank256 `bs16`, LR `3e-4`, 1k run reached target/source `1.683542 / 3.076128`, train loss `1.670848`, state `192.4 MB`, peak `3.23 GB`, and `~14.2k tok/s`. Adding per-matrix projected-gradient clipping at `2.0` improved the same shape to `1.675010 / 3.059717`, train `1.660258`, with mean pre-clip max projected tensor norm `1.088935` and unchanged state/memory class. This is the strongest target lane so far at similar walltime/token budget. It suggests rank capacity and optimizer-update count matter more than simply maximizing batch size.
- **Grassmann refresh is now scale-controlled like basis init.** Basis init already normalized the full gradient before side-Gram `eigh`; Grassmann refresh now normalizes the spectral input before tangent formation too. Unit tests assert right- and left-side refreshes are invariant to multiplying the refresh gradient by `1000×`. A rank256 `bs16`, LR `3e-4`, 200-step smoke after the change reached target/source `1.748007 / 3.020345`, essentially matching the previous same-lane step200 `1.748073 / 3.022887`, so this is a hygiene/invariant fix rather than an immediate quality jump.
- **Projected-gradient clipping belongs after projection, before the projected EMA.** A stressed rank64 `bs32`, LR `4e-4` lane showed raw/projected gradient spikes without a chordal/basis event: one diagnostic run hit pre-clip projected tensor max `109486`, raw grad norm `773670`, and then poisoned the projected moment tail. Clipping each projected matrix gradient to norm `1.0` removed the rank64 failure and improved target (`1.728171` vs `1.796161` in the diagnostic rerun), but touched normal rank256 traffic. A fixed threshold `2.0` keeps the rank256 default lane breathing while still crushing pathological spikes. Do not clip raw full gradients before projection.
...
- **Default-candidate lane: repaired checkpointing, residual-facing, no activation projection, rank256, likely `bs16`.** The old `bs8/rank64` lane is continuity history. The best current 350M shape is rank256 with enough optimizer updates, not maximum batch size. Current best candidate is `bs16 × seq1024`, rank256, LR `3e-4`, repaired checkpointing, residual-facing, burst refresh, and per-matrix projected-gradient clip `2.0`. Refine this before larger models or product data.
- **Watch Grassmann behavior after scale-control, but do not overfit the first smoke.** The refresh-input normalization is now implemented and passed a 200-step rank256 sanity run without changing the lane materially. The remaining question is longer-horizon source/target behavior and whether chordal motion/source drift improve once LR/beta/moment precision are tuned, not whether raw gradient scale should leak into the basis step.
- **Leave projected moments bf16 unless long high-beta runs prove otherwise.** The first fp32 `projected_exp_avg` migration did not improve the 200-step default-candidate sensor and cost `~144 MB` extra matrix state at rank256. Basis storage should also remain param dtype: init/refresh already use fp32 work internally, while fp32 bases would double basis memory/matmul traffic and answer a different question.
- **Projected activation is now a sidequest, documented in `PROJECTED_ACTIVATION.md`.** At `2e-4`, repaired-checkpoint full `lfm` activation projection and a no-activation all-right control match closely at 200 steps, separating custom backward math from side-policy geometry. Under real layer checkpointing, projected activation cuts peak from `~2.07 GB` to `~1.80 GB` at `bs8 × seq1024`, but eager wrappers slow steps from `~0.524s` to `~0.571s`. It is faithful and may be optimized later, but the mainline is repaired checkpointing plus residual-facing SumoTrack.
