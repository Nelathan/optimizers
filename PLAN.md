...
- **Re-evaluate token mass after the checkpoint repair.** Full `lfm` projected activation with repaired checkpointing fits short `seq1024` smokes at `bs16`, `bs32`, `bs64`, and a one-step `bs80` on the local card (`bs80` peak allocated `~9.93 GB`, reserved `~10.71 GB`). These are not quality claims. They mean the old `bs8` memory boundary is obsolete; the next useful runs should choose token mass by target/source behavior and throughput, not fear of the previous fake checkpoint wall.
