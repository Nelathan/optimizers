...
Current status: Aurora-style leverage-uniform polar is the projected direction map. Muon scale semantics are fixed, not a user-facing scale-mode knob. Leverage diagnostics remain an internal/Aurora-investigation tool, but they are no longer routine harness telemetry; if the sauce amount becomes the question again, add a deliberately named diagnostic cut instead of reopening wandb clutter. Packed SDPA batches, the CCE harness path, and Aurora cycle knobs are wired through the smoke harness; the remaining question is whether they matter at realistic token counts rather than tiny smokes. Qwen3.5-2B-Base plus `flash-linear-attention` is now present, and `causal-conv1d` is now buildable in this repo environment. The repo floor is pinned to Python 3.13 via `.python-version`.
...
4. **Basis movement remains useful, but keep the sensor small.** Routine telemetry should keep only chordal basis rotation. Projection-capture before/after refresh was unhelpful in the first telemetry runs and should not return unless it answers a sharper failure mode. Do not revive broad projected-residual telemetry as metric gardening.
...
10. **Gradient accumulation is currently a memory/token-budget tool, not a quality fix.** The same-token `grad_accum=2` check at LR `2e-4` reduced raw grad norm but lost target/source quality against the non-accumulated baseline. Reopen accumulation only when memory forces it or when testing a deliberately scaled LR/update-count contract.
11. **First-basis quality from accumulated gradients.** Separate ordinary training accumulation from basis-initialization accumulation. A useful probe is “initialize `eigh` bases from 4 microbatches, then train normally” versus ordinary first-batch basis init. This answers whether the first spectral basis is noisy at low tokens/step without changing the whole training contract.
12. **Basis rotation telemetry.** Keep chordal distance between old and new basis as a core cadence metric. It is cheap enough because it only runs on refresh/cadence and uses rank×rank basis overlap. Do not compile the whole refresh path; if refresh ever profiles hot, split and compile a pure tensor refresh primitive instead.
13. **Aurora / Newton-Schulz cycle economy.** Measure curve quality and step time for cheaper projected orthogonalization, especially `aurora_pp_iterations=1` with `polar_ns_steps=2` or `3`, against the current default `2/5`. Low tokens/step makes optimizer compute spend matter, but small GPUs are usually more VRAM-bound than compute-bound; the target is “just enough orthogonalization,” not perfect polar math.
14. **Frobenius normalization and Gram symmetrization are default eigensolver hygiene.** For one-time basis init, keep them as default with no flag garden: scalar Frobenius normalization preserves the dominant subspace and protects scale; Gram symmetrization enforces the exact symmetric object intended by `GᵀG`/`GGᵀ`. Re-evaluate cost only if similar symmetrization moves into a frequent tracking path.
...
1. **Use the faithful `bs8 + checkpointing`, LR `2e-4` lane as the current small-model quality baseline.** The broad 1k sweep found the knee in `1e-4–3e-4`; the direct `2e-4` run landed well enough to use as the working baseline while other axes are tested.
2. **Compile only honest hot surfaces.** `--torch-compile` now means model forward/backward plus SumoTrack's pure tensor orthogonalization kernel. Whole `optimizer.step()` compile failed structurally on parameter/state bookkeeping and HeavyBall fallback internals; do not reintroduce it casually.
3. **Grad accumulation is not a quality fix yet.** A same-token `grad_accum=2`, 500-step check at LR `2e-4` reduced raw grad norm but lost on target and slightly on source versus the 1000-step non-accumulated baseline at the same update scale. Keep grad accumulation available as a memory/token-budget tool, not as the next quality hypothesis.
4. **Keep Aurora/NS defaults for baseline continuity.** The 2e-4 cycle sweep was too noisy for a default change. Conservative read: `pp=1` is probably safe, `ns=1` is too cheap, `ns=5` remains the baseline.
5. **Keep wandb core small.** Routine graphs are target/source validation loss, train loss, raw grad norm, update norm, update/param, and basis rotation chordal. No final summaries, no basis capture, no leverage, no memory graphs from the harness.
6. **Use Qwen3.5-2B-Base only as a calibration probe unless explicitly promoted.** OOM-safe CCE probes at `batch=1`, short sequence length are enough to answer whether LFM is uniquely high-gradient on the same text. Do not turn Qwen into the main benchmark lane by accident.
7. **If `bs16`-equivalent signal is needed, use grad accumulation or real memory cuts.** Checkpointing got `batch_size=8`, but the card remains effectively full; do not assume another batch-size rung exists.
8. **Throughput can wait, but remains a product lead.** Realistic-token Aurora amortization remains useful; it is just not the next cut unless the user explicitly reopens throughput. Current uncertainty is quality/source behavior under the faithful lane, not launch overhead.

## Next session contract

Start from the faithful-format 1k comparison, LR sweep, direct `2e-4` baseline, grad-accumulation check, and basis-rotation cleanup, not from throughput. Residual-facing side, uniform rank 64, stable `eigh` basis init, CCE, faithful right-padded SYNTH batches, SDPA attention, targeted tensor-kernel compile, and Aurora remain the quality/diagnostic harness mainline. EOS-packed no-mask remains a throughput lane. The active unknown is quality/source behavior under this cleaner lane, not whether packed batching, frequent refresh, or metric sprawl fixes the issue.
...
1. Use `--torch-compile` for future 1k small-model quality runs unless it changes the experiment contract; this compiles model forward/backward and SumoTrack tensor kernels, not the Python optimizer step.
2. Keep default `basis_refresh_interval=100` unless basis-rotation telemetry proves refresh cadence is the bottleneck.
3. Keep wandb to the core metric set. If another metric is needed, name the hypothesis before adding the graph.
