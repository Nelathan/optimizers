...
| Where does compiled optimizer walltime go? | the mature Oja + Adafactor + projected-EMA preparation cut compiles faithfully while Python owns state and phase-two Oja/Aurora stays globally batched. At LFM-1.2B it reduced step time from `2.384706` to `2.223879` seconds (6.7%); a matched 350M replay was loss-identical and, after its one-time 3.4-second compile toll, ran the final 75 steps about 4.4% faster | keep the compiled preparation cut; its startup toll amortizes around 120 steps at 350M, so short mechanism sensors should remain eager |
...
| Does UsuiTrack beat the memory-matched LoRA user alternative? | the single reviewed LoRA arm targets exactly the same non-embedding matrix set at rank 44, alpha 44, bf16 adapters and bf16 AdamW `.9/.99` state, global adapter clip 1, and the shared fp32 fallback policy. Its matrix-side state is `98,943,712` bytes versus UsuiTrack rank 128's `97,443,840`. The LR sensor selected `2e-4`; at 1k, LoRA run `morwmsaf` reached `1.674265 / 3.084880` target/source, while the directly matched current UsuiTrack run `qvtpskn2` reached `1.672156 / 3.037444`. LoRA led target strongly at step 100 (`1.769886` vs `1.876109`) but its advantage narrowed monotonically through step 800; UsuiTrack crossed at step 900 and finished ahead. UsuiTrack also used `2,882,331,648` peak allocated bytes versus LoRA's `3,807,851,520`, while LoRA was only about 1% faster (`.751236` vs `.758465` sec/step). | yes at the current 1k product gate: UsuiTrack finishes slightly better on target, materially better on source, with 1.5% less persistent optimizer/adapter state and 24.3% less peak allocated VRAM. The curve is consistent with the moving-rank-capacity premise: LoRA adapts faster early, while UsuiTrack keeps gaining and overtakes late. |
...
The primary controlled LFM-350M LoRA+AdamW comparison is complete under
identical formatting, token budget, compile state, evaluation cadence, and
shared fallback policy. UsuiTrack passed on target/source, state, and peak VRAM;
the exact evidence is recorded in the research map. This clears the core
algorithmic product gate for an open-source release and whitepaper-style
repository document. Generic PyTorch integration gates remain engineering work,
not a reason to blur the benchmark result.
