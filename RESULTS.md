...
| rank256 dual-rail projected-clip | completed | `bs16 × 1000` | `3e-4` | `6abrqjmo` | `1.674212` | `3.059725` | `1.660581` | `0.094799` | `0.354311` | `192,403,456` | `3,226,123,264` | `13,604` |
| rank256 dual-rail LR warmup | completed | `bs16 × 200` | `3e-4` | `vn1i5uhm` | `1.756553` | `3.017035` | `1.807934` | `0.088776` | `0.424264` | `192,403,456` | `3,015,179,264` | `13,405` |
| rank256 dual-rail LR pressure | completed | `bs16 × 200` | `4e-4` | `04x0tkq6` | `1.736504` | `3.038449` | `1.789199` | `0.117525` | `0.423801` | `192,403,456` | `3,015,179,264` | `13,569` |
| rank256 loose-rail LR pressure | completed | `bs16 × 200` | `6e-4` | `bn1f18bu` | `1.744359` | `3.095776` | `1.794771` | `0.169315` | `0.423536` | `192,403,456` | `3,015,179,264` | `13,565` |
| rank256 high-beta dual-rail | killed after step 300 | `bs16 × 300` | `3e-4` | `x42mxid1` | `1.756316` | `3.166636` | `1.693017` | `0.082529` | `0.366909` | n/a | n/a | n/a |
| rank256 high-beta no-ratio control | killed after step 400 | `bs16 × 400` | `3e-4` | `t7jqwb09` | `1.715256` | `3.104252` | n/a | n/a | n/a | n/a | n/a | n/a |
| rank256 high-beta beta-warmup backtest | rejected | `bs16 × 200` | `3e-4` | `ur7p1c4t` | `1.779715` | `3.182923` | `1.829517` | `0.091347` | `0.423830` | `192,403,456` | `3,015,179,264` | `13,451` |
| rank256 refresh20 check | stopped after step 300 | `bs16 × 300` | `3e-4` | `votr0yt3` | `1.725838` | `3.036959` | n/a | n/a | n/a | n/a | n/a | n/a |
...
- Projected-gradient clipping belongs after projection and before the projected EMA. The diagnostic rank64 stress run without clipping (`17xa2n7q`) exposed a raw/projected gradient outlier at step 492: raw grad norm `773670.5`, pre-clip projected tensor max `109486.6`, no chordal event, then a poisoned moment/update tail. Per-matrix projected clipping at `1.0` removed the rank64 failure mode and improved the stressed lane's final target, but rank256 normal traffic had mean pre-clip max above `1`. A fixed norm cap `2.0` kept normal rank256 traffic breathing and improved both target/source against the unclipped `bekcxmma` lane.
- The projected-gradient spectrum showed tensor norms are heterogeneous, while projected-grad/moment ratios are the better pressure gauge. The harness now has dual rails: absolute projected norm plus tensor-local projected-grad/moment ratio. A tight ratio `5` did not move the 1k loss needle versus norm-only clipping (`1.674212 / 3.059725` vs `1.675010 / 3.059717`) and logged a clean p90-ratio lane (`mean=4.693210`), but telemetry suggests `5` is a little tight; default ratio is `6` for breathing room.
- LR warmup is a clear default win for the current rank256 lane. With `50` warmup steps, dual rails `norm=2/ratio=6`, and LR `3e-4`, the 200-step sensor reached `1.756553 / 3.017035` with lower source loss, lower update norm (`0.088776`), and lower mean p90 ratio (`3.918923`) than the no-warmup 200-step dual-rail lane (`1.747635 / 3.022103`, update norm `0.097216`, p90 ratio `4.397594`). The target is behind at step 200 because the first 50 steps are intentionally throttled; the cleaner source/pressure profile is worth making warmup default before longer runs.
- LR pressure tests show the regulator is a guardrail, not magic. `4e-4` improved 200-step target (`1.736504`) at moderate source cost (`3.038449`) and is useful as a rough-track stability probe. `6e-4` with looser rails failed to improve target (`1.744359`) and substantially worsened source (`3.095776`), so it is wheelspin, not a better convergence lane.
- Beta `0.98` is rejected for the current cold-start harness. With dual rails, source was bad from step 100 (`1.825120 / 3.134735`) and p90 projected-grad/moment ratios exploded (`47×` at step 1, `30933×` at the step194 spike). Disabling ratio clipping did not rescue high beta: the no-ratio control was still source-bad from step 100 (`1.804987 / 3.094521`) through step 400 (`1.715256 / 3.104252`). The problem is slow projected moments plus Aurora/Muon early trajectory, not simply the ratio rail. LR warmup is still likely needed as general hygiene, but high beta would need beta warmup or mature/bias-corrected moment handling too.
- A running-average beta warmup backtest (`beta_t=min(beta, t/(t+1))`) did not rescue high beta. With `beta=0.98`, LR warmup `50`, norm clip `2`, and no ratio rail, the 200-step run landed at `1.779715 / 3.182923`, with final p90 ratio `10.33`. The schedule makes early pressure look mathematically cleaner, but high beta remains too laggy for this projected/Aurora path. The code branch was removed; LR warmup remains the boring default.
- Refresh interval `20` did not separate from interval `100` in the current rank256 lane. Through step 300 it overlapped the default run (`1.725838 / 3.036959` vs `1.725741 / 3.040125`), so the run was stopped rather than spending more compute on basis churn. Tune Grassmann update behavior if basis adaptation remains suspect; do not refresh more often by default.
...
- Full unit suite passed before the first LFM smoke: `63 tests OK`; after layer-staggered refresh support: `66 tests OK`; after extending `lfm` activation projection to attention/short-conv projection Linears: `68 tests OK`; after corrected-LR/default and wrapper-math tests: `69 tests OK`; after fused MLP recompute scheduling: `70 tests OK`; after LFM2 checkpoint repair tests: `72 tests OK`.
...
| run | backend | steps | shape | peak allocated CUDA | tokens/s | step sec | notes |
| --- | --- | ---: | --- | ---: | ---: | ---: | --- |
| tiny compare | `off` | 1 | `bs1 × seq512` | `1,376,094,720` | `8,884` | `0.0576` | ordinary MLP backward |
...
| same-shape baseline | `off` | 10 | `bs4 × seq1024` | `5,243,818,496` | `20,921` | `0.1958` | current harness baseline, no activation projection |
...
One-forward/backward monitor at `bs8 × seq1024`, full `lfm` backend, one warm basis step, then measured forward/backward:

| mode | after forward+loss allocated | final peak allocated | queued projected grads after backward | retained full grads after backward | notes |
| --- | ---: | ---: | ---: | ---: | --- |
| no checkpoint | `4,370,416,640` | `4,731,528,192` | `28,573,696` | `128,512` | refined monitor before checkpoint repair |
...
| repaired checkpoint full activation projection | `lfm`, layer-staggered, `2e-4` | 200 | `1.861770` | `3.004450` | `1.849765` | `0.059228` | `1,797,460,480` | W&B `51nsznhn`; real LFM2 decoder-layer checkpointing active |
| repaired checkpoint residual-facing baseline | `off`, residual-facing, burst, `2e-4` | 200 | `1.877504` | `2.991532` | `1.865052` | `0.058456` | `2,072,345,088` | W&B `dob9ref7`; same repaired checkpoint substrate |
| repaired checkpoint all-right control | `off`, right side policy, burst, `2e-4` | 200 | `1.861207` | `3.005637` | `1.847264` | `0.058220` | `2,072,345,088` | W&B `0kco6hhh`; isolates side geometry from projected backward |
...
The same-shape no-activation-projection baseline reported installed modules `0`, side-policy left/right/auto tensors `32/50/10`, state bytes `48,486,400`, actual sequence tokens/update `4096`, supervised tokens/update `3340`, elapsed `1.958s`, peak reserved CUDA `5,448,400,896`, and last measured train loss `1.945335`.
...
After the LFM2 checkpoint repair, the corrected-LR full `lfm` 200-step run reported W&B run `51nsznhn`, installed modules `60`, elapsed `114.177s`, step sec `0.570883`, tokens/s `14,349.692`, peak reserved CUDA `2,472,542,208`, last measured train loss `1.849765`, mean grad norm `1.633488`, mean update norm `0.059228`, update/param `0.000239`, mean basis rotation chordal `0.177008`, and source delta `+0.105012`.

The repaired-checkpoint residual-facing `off` control reported W&B run `dob9ref7`, step sec `0.524733`, tokens/s `15,611.746`, peak allocated `2,072,345,088`, target/source `1.877504 / 2.991532`, mean update norm `0.058456`, and source delta `+0.092094`. The repaired-checkpoint all-right `off` control reported W&B run `0kco6hhh`, step sec `0.524142`, tokens/s `15,629.357`, peak allocated `2,072,345,088`, target/source `1.861207 / 3.005637`, mean update norm `0.058220`, and source delta `+0.106210`.

Short boundary smokes with repaired checkpointing, full `lfm`, corrected LR, validation skipped, `seq_len=1024`:

| batch | measured steps | tokens/update | peak allocated CUDA | peak reserved CUDA | tokens/s | step sec | notes |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 16 | 10 | `16,384` | `2,609,973,760` | `2,990,538,752` | `14,085` | `1.1632` | all-right `off` control at same shape peaked `2,817,230,336` and ran `1.0608s`/step |
| 32 | 5 | `32,768` | `4,439,957,504` | `5,087,690,752` | `13,957` | `2.3478` | boundary smoke only |
| 64 | 2 | `65,536` | `8,099,395,072` | `9,386,852,352` | `13,904` | `4.7135` | close to card pressure but fits |
| 80 | 1 | `81,920` | `9,928,586,752` | `10,710,155,264` | `13,861` | `5.9103` | near practical 12GB wall; stop boundary search here |
...
- The repaired LFM2 checkpoint path is the largest memory cut so far on the `bs8 × seq1024` monitor. Stock `--activation-checkpointing` was effectively a no-op for LFM2 decoder layers; the local repair dropped the one-step peak to `~1.61 GiB` and after-forward allocation to `~1.04 GiB`, which is the flat curve expected from real layer checkpointing.
- The repaired LFM2 checkpoint path also holds in a 200-step corrected-LR quality sensor: target/source at step 200 were `1.861770 / 3.004450`, comparable to the earlier corrected run, while allocated peak fell to `1.80 GB`. The trade is throughput: step time rose to `0.5709s` from the old no-op checkpoint lane's `~0.42s`.
- Under real checkpointing, projected activation still buys memory but the incremental win is now modest: full `lfm` projection peaks at `1.80 GB` versus `2.07 GB` for both repaired-checkpoint `off` controls. The all-right no-projection control matches full `lfm` quality almost exactly, so projected activation remains a faithful implementation of all-right geometry; the residual-facing baseline keeps source slightly better and learns target slightly slower. Current eager projected wrappers cost throughput (`0.5709s` vs `0.5241s` per step against all-right `off`).
- The repaired checkpoint substrate changes the scale question. With full `lfm` projected activation, `bs16`, `bs32`, `bs64`, and even a one-step `bs80` smoke fit at `seq1024` on the local card. These are not quality claims, but they show the path has moved from “can bs8 survive?” to “what token mass is actually useful before throughput/quality tradeoffs dominate?”
...
- Use corrected-LR target/source curves as regression sensors while cutting memory; rerun them on the repaired checkpoint substrate before making quality or throughput claims.
